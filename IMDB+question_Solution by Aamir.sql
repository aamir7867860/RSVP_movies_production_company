...
Name    	:Aamir
Course   	:= UpGrad Campus DSA July 2022 Certification Program
Batch   	:= 4401                                                                                                */
...
+---------------+-------------------+ */
-- Type your code below:
select Year,count(year) 'number_of_movies' 
from movie group by year;

select month(date_published) 'month_num',count(month(date_published)) 'number_of_movies' 
...
where g.movie_id=m.id group by g.genre limit 1;
...
where g.movie_id=m.id group by g.genre;
...
select * from 
    ...
    from movie,ratings 
    where movie.id=ratings.movie_id) as title_table
...
-- Type your code below:
select Germany_Votes,Italy_Votes,case 
...
select name,movie_count from 
    ...
    where avg_rating>8 and genre in 
        (select genre from 
            ...
            where avg_rating>8
            group by genre order by count(avg_rating) desc limit 3) as top3
...
as dir_summary
group by director_id order by number_of_movies desc,director_name
limit 9;
